...
import AdminAttendancePage from './adminattendancepage.jsx';
import AdminAssignmentPage from './adminassignmentpage.jsx';
import AdminExamPage from './adminexampage.jsx';
...
function AdminClassroomPage({ classId, adminUser, onBack }) {
  ...
  const [feeActionBusyKey, setFeeActionBusyKey] = useState('');
  ...
  async function handleAddStudent(event) {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/admin/classes/${classId}/students/create`, {
        method: 'POST',
  ...
  async function handleDeleteClass() {
    if (!window.confirm(`Delete ${classroom?.className} - ${classroom?.section}? This cannot be undone.`)) {
      return;
    }

    setBusyKey('delete-class');
    setError('');
    setFeedback('');

    try {
      const encodedAdminEmail = encodeURIComponent(adminUser.email);
      const response = await fetch(`${API_BASE_URL}/api/admin/classes/${classId}?adminEmail=${encodedAdminEmail}`, {
        method: 'DELETE'
      });
      const data = await response.json();
  ...
  async function openStudentDetails(studentEmail) {
    ...
    try {
      ...
      const encodedAdminEmail = encodeURIComponent(adminUser.email);
      const response = await fetch(`${API_BASE_URL}/api/students/${encodedStudentEmail}/details?adminEmail=${encodedAdminEmail}`);
      const data = await response.json();
  ...
  async function handleMarkFeePaid(fee) {
    if (!selectedStudentDetails?.student?.email) {
      return;
    }

    const feeKey = `${fee.title}-${fee.dueDate}`;
    setFeeActionBusyKey(feeKey);
    setFeeActionError('');
    setFeeActionMessage('');

    try {
      const encodedStudentEmail = encodeURIComponent(selectedStudentDetails.student.email);
      const response = await fetch(`${API_BASE_URL}/api/students/${encodedStudentEmail}/fees/pay`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          adminEmail: adminUser.email,
      ...
      if (!response.ok) {
        setFeeActionError(data.message || 'Unable to mark fee as paid.');
        return;
      }
      ...
      setSelectedStudentDetails((currentDetails) => {
        ...
        return {
          ...currentDetails,
          feeDetails: currentDetails.feeDetails.map((currentFee) =>
            currentFee.title === fee.title && currentFee.dueDate === fee.dueDate
              ? data.fee
              : currentFee
          )
        };
      });
      setFeeActionMessage(data.message);
      await loadClassroom();
    } catch (requestError) {
      setFeeActionError('Could not connect to the backend server.');
    } finally {
      setFeeActionBusyKey('');
    }
  }
  ...
  async function handleRemoveStudent(studentEmail) {
    ...
    try {
      const encodedAdminEmail = encodeURIComponent(adminUser.email);
      const encodedStudentEmail = encodeURIComponent(studentEmail);
      const response = await fetch(`${API_BASE_URL}/api/admin/classes/${classId}/students/${encodedStudentEmail}?adminEmail=${encodedAdminEmail}`, {
        method: 'DELETE'
      });
      const data = await response.json();
  ...
  function sectionNavClass(section) {
    return currentSection === section
      ? 'rounded-full bg-white px-4 py-2 text-sm font-semibold text-orange-800'
  ...
  return (
    <div className="min-h-screen bg-orange-50 p-5">
      <div className="mx-auto max-w-7xl">
        <BrandBanner subtitle="Admin Class Page" textClassName="text-orange-900" subtextClassName="text-orange-700" className="mb-4" />
        <button
          type="button"
          onClick={onBack}
        ...
        <button
          ...
          disabled={busyKey === 'delete-class'}
          className="mb-4 ml-3 rounded-lg border border-rose-200 bg-rose-50 px-4 py-2 text-sm font-medium text-rose-700 transition hover:bg-rose-100 disabled:bg-rose-100 disabled:text-rose-400"
        >
          {busyKey === 'delete-class' ? 'Deleting Class...' : 'Delete Class'}
        </button>

        {loading ? <p className="rounded-lg border border-orange-200 bg-orange-50 px-3 py-2 text-sm text-orange-700">Loading class page...</p> : null}
        ...
        {classroom ? (
          <>
            <section className="rounded-xl bg-slate-900 p-5 text-white">
              <p className="text-sm font-semibold uppercase tracking-[0.2em] text-orange-300">Class Page</p>
              <h1 className="mt-3 text-4xl font-bold">{classroom.className}</h1>
              <p className="mt-2 text-slate-200">{classroom.subject}</p>
              <p className="mt-4 text-sm text-slate-300">Teacher: {classroom.teacher?.name} ({classroom.teacher?.email})</p>
              <p className="mt-2 text-sm text-slate-300">{classroom.students.length} students enrolled</p>
            ...
            <section className="mt-4 grid gap-4 xl:grid-cols-2">
              <form className={currentSection === 'students' ? 'card-scrollbar max-h-[44rem] overflow-y-auto rounded-xl bg-white p-4 shadow-sm' : 'hidden'} onSubmit={handleAddStudent}>
                <h2 className="text-2xl font-semibold text-slate-900">Add Student To This Class</h2>
                <div className="mt-3 space-y-4">
                  <input type="text" value={studentName} onChange={(event) => setStudentName(event.target.value)} placeholder="Student full name" className="w-full rounded-lg border border-slate-200 px-3 py-2 outline-none focus:border-orange-500 focus:ring-4 focus:ring-orange-100" />
                  <input type="email" value={studentEmail} onChange={(event) => setStudentEmail(event.target.value)} placeholder="student@schoolerp.com" className="w-full rounded-lg border border-slate-200 px-3 py-2 outline-none focus:border-orange-500 focus:ring-4 focus:ring-orange-100" />
                  <input type="password" value={studentPassword} onChange={(event) => setStudentPassword(event.target.value)} placeholder="Temporary password" className="w-full rounded-lg border border-slate-200 px-3 py-2 outline-none focus:border-orange-500 focus:ring-4 focus:ring-orange-100" />
                  <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
                    ...
                    <div className="mt-4 space-y-3">
                      <input type="text" value={studentFeeAmount} onChange={(event) => setStudentFeeAmount(event.target.value)} placeholder="Amount" className="w-full rounded-lg border border-slate-200 px-3 py-2 outline-none focus:border-orange-500 focus:ring-4 focus:ring-orange-100" />
                      <input type="date" value={studentFeeDueDate} onChange={(event) => setStudentFeeDueDate(event.target.value)} className="w-full rounded-lg border border-slate-200 px-3 py-2 outline-none focus:border-orange-500 focus:ring-4 focus:ring-orange-100" />
                      <input type="number" min="1" value={studentFeeMonths} onChange={(event) => setStudentFeeMonths(event.target.value)} placeholder="Number of months" className="w-full rounded-lg border border-slate-200 px-3 py-2 outline-none focus:border-orange-500 focus:ring-4 focus:ring-orange-100" />
              ...
              <div className={currentSection === 'announcements' ? 'rounded-xl bg-white p-4 shadow-sm xl:col-span-2' : 'hidden'}>
              ...
              <div className={currentSection === 'notes' ? 'rounded-xl bg-white p-4 shadow-sm xl:col-span-2' : 'hidden'}>
                            ...
                            <div className="mt-3 flex flex-wrap gap-2">
                              {note.attachments.map((attachment) => (
                                <a
                                  ...
                                  className="rounded-xl border border-orange-200 bg-orange-50 px-3 py-2 text-xs font-medium text-orange-800 transition hover:bg-orange-100"
                                >
              ...
              <form className={currentSection === 'exams' ? 'rounded-xl bg-white p-4 shadow-sm' : 'hidden'} onSubmit={handleScheduleExam}>
                <h2 className="text-2xl font-semibold text-slate-900">Schedule Exam In Class</h2>
                <div className="mt-3 space-y-4">
                  <input type="text" value={examSubject} onChange={(event) => setExamSubject(event.target.value)} placeholder="Exam subject" className="w-full rounded-lg border border-slate-200 px-3 py-2 outline-none focus:border-orange-500 focus:ring-4 focus:ring-orange-100" />
                  <input type="date" value={examDate} onChange={(event) => setExamDate(event.target.value)} className="w-full rounded-lg border border-slate-200 px-3 py-2 outline-none focus:border-orange-500 focus:ring-4 focus:ring-orange-100" />
                  <input type="time" value={examTime} onChange={(event) => setExamTime(event.target.value)} className="w-full rounded-lg border border-slate-200 px-3 py-2 outline-none focus:border-orange-500 focus:ring-4 focus:ring-orange-100" />
                  <input type="text" value={examRoom} onChange={(event) => setExamRoom(event.target.value)} placeholder="Room" className="w-full rounded-lg border border-slate-200 px-3 py-2 outline-none focus:border-orange-500 focus:ring-4 focus:ring-orange-100" />
                  <input type="number" min="1" value={examMaxMarks} onChange={(event) => setExamMaxMarks(event.target.value)} placeholder="Max marks" className="w-full rounded-lg border border-slate-200 px-3 py-2 outline-none focus:border-orange-500 focus:ring-4 focus:ring-orange-100" />
                  <button type="submit" disabled={busyKey === 'exam'} className="w-full rounded-lg bg-orange-600 px-3 py-2 font-semibold text-white transition hover:bg-orange-700 disabled:bg-orange-300">
                    {busyKey === 'exam' ? 'Scheduling Exam...' : 'Schedule Exam'}
                  </button>
                </div>
              </form>

              <div className={currentSection === 'exams' ? 'rounded-xl bg-white p-4 shadow-sm' : 'hidden'}>
                <h2 className="text-2xl font-semibold text-slate-900">Scheduled Exams</h2>
                <div className="mt-3 h-96 space-y-3 overflow-y-scroll pr-2">
                  {orderedScheduledExams.length ? orderedScheduledExams.map((exam) => (
                    <button
                      type="button"
                      key={`${exam.subject}-${exam.date}-${exam.time}`}
                      onClick={() => setOpenedExam(exam)}
                      className="w-full rounded-lg border border-slate-200 p-4 text-left transition hover:border-orange-300 hover:bg-orange-50"
                    >
                      ...
                      <p className="mt-3 text-sm font-medium text-orange-700">Open exam page</p>
                    </button>
                  )) : <p className="text-sm text-slate-500">No class exams have been scheduled yet.</p>}
                </div>
              </div>

              <form className={currentSection === 'assignments' ? 'rounded-xl bg-white p-4 shadow-sm' : 'hidden'} onSubmit={handleScheduleAssignment}>
                <h2 className="text-2xl font-semibold text-slate-900">Schedule Assignment In Class</h2>
                <div className="mt-3 space-y-4">
                  <input type="text" value={assignmentTitle} onChange={(event) => setAssignmentTitle(event.target.value)} placeholder="Assignment title" className="w-full rounded-lg border border-slate-200 px-3 py-2 outline-none focus:border-orange-500 focus:ring-4 focus:ring-orange-100" />
                  <input type="text" value={assignmentSubject} onChange={(event) => setAssignmentSubject(event.target.value)} placeholder="Subject" className="w-full rounded-lg border border-slate-200 px-3 py-2 outline-none focus:border-orange-500 focus:ring-4 focus:ring-orange-100" />
                  <input type="datetime-local" value={assignmentDeadline} onChange={(event) => setAssignmentDeadline(event.target.value)} className="w-full rounded-lg border border-slate-200 px-3 py-2 outline-none focus:border-orange-500 focus:ring-4 focus:ring-orange-100" />
                  <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
                  ...
                  <button type="submit" disabled={busyKey === 'assignment'} className="w-full rounded-lg bg-orange-600 px-3 py-2 font-semibold text-white transition hover:bg-orange-700 disabled:bg-orange-300">
                    {busyKey === 'assignment' ? 'Scheduling Assignment...' : 'Schedule Assignment'}
                  </button>
                </div>
              </form>

              <div className={currentSection === 'assignments' ? 'rounded-xl bg-white p-4 shadow-sm' : 'hidden'}>
                <h2 className="text-2xl font-semibold text-slate-900">Scheduled Assignments</h2>
                <div className="mt-3 h-96 space-y-3 overflow-y-scroll pr-2">
                  {classroom.scheduledAssignments?.length ? classroom.scheduledAssignments.map((assignment) => (
                    <button
                      type="button"
                    ...
                    >
                      ...
                      <p className="mt-3 text-sm font-medium text-orange-700">Open assignment details</p>
                    </button>
                  )) : <p className="text-sm text-slate-500">No class assignments have been scheduled yet.</p>}
                </div>
              </div>

              <form className={currentSection === 'announcements' ? 'rounded-xl bg-white p-4 shadow-sm xl:col-span-2' : 'hidden'} onSubmit={handlePostAnnouncement}>
                <h2 className="text-2xl font-semibold text-slate-900">Make Announcement In Class</h2>
                <div className="mt-3 space-y-4">
                  <input type="text" value={announcementTitle} onChange={(event) => setAnnouncementTitle(event.target.value)} placeholder="Announcement title" className="w-full rounded-lg border border-slate-200 px-3 py-2 outline-none focus:border-orange-500 focus:ring-4 focus:ring-orange-100" />
                  <textarea value={announcementMessage} onChange={(event) => setAnnouncementMessage(event.target.value)} placeholder="Write announcement for the class" rows="4" className="w-full rounded-lg border border-slate-200 px-3 py-2 outline-none focus:border-orange-500 focus:ring-4 focus:ring-orange-100" />
              ...
              <form className={currentSection === 'notes' ? 'rounded-xl bg-white p-4 shadow-sm xl:col-span-2' : 'hidden'} onSubmit={handleAddNote}>
                <h2 className="text-2xl font-semibold text-slate-900">Add Note In Class</h2>
                <div className="mt-3 space-y-4">
                  <input type="text" value={noteTitle} onChange={(event) => setNoteTitle(event.target.value)} placeholder="Note title" className="w-full rounded-lg border border-slate-200 px-3 py-2 outline-none focus:border-orange-500 focus:ring-4 focus:ring-orange-100" />
                  <textarea value={noteContent} onChange={(event) => setNoteContent(event.target.value)} placeholder="Write note for this class" rows="4" className="w-full rounded-lg border border-slate-200 px-3 py-2 outline-none focus:border-orange-500 focus:ring-4 focus:ring-orange-100" />
                  <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
                    <label className="block text-sm font-medium text-slate-700">
                      Attach PDF or files
                  ...
                  <button type="submit" disabled={busyKey === 'note'} className="w-full rounded-lg bg-orange-600 px-3 py-2 font-semibold text-white transition hover:bg-orange-700 disabled:bg-orange-300">
                    {busyKey === 'note' ? 'Adding Note...' : 'Add Note'}
                  </button>
                </div>
              </form>

              <div className={currentSection === 'students' ? 'rounded-xl bg-white p-4 shadow-sm xl:col-span-2' : 'hidden'}>
                <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                  ...
                  <input
                    ...
                    className="w-full rounded-lg border border-slate-200 px-3 py-2 outline-none focus:border-orange-500 focus:ring-4 focus:ring-orange-100 lg:max-w-md"
                  />
                </div>
                <div className="mt-3 space-y-4">
                  {filteredStudents.length ? filteredStudents.map((student) => (
                    <div key={student.email} className="space-y-4">
                      <div className="block w-full rounded-lg border border-slate-200 p-4 transition hover:border-orange-300 hover:bg-orange-50">
                        <div className="flex items-start justify-between gap-4">
                          ...
                          <div className="flex flex-col items-end gap-3">
                            ...
                            <button
                              ...
                              className="rounded-xl border border-rose-200 bg-rose-50 px-3 py-2 text-xs font-semibold text-rose-700 transition hover:bg-rose-100 disabled:bg-rose-100 disabled:text-rose-400"
                            >
                      ...
                      {(
                        ...
                        <StudentDetailsPanel
                          ...
                          feeActionBusyKey={feeActionBusyKey}
                          feeActionMessage={feeActionMessage}
                          feeActionError={feeActionError}
                          onClose={() => {
                            setOpenedStudentEmail('');
                            setSelectedStudentDetails(null);
                            setStudentDetailsError('');
