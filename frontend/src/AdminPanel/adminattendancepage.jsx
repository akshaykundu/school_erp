  ...
  return (
    <div className="min-h-screen bg-orange-50 p-5">
      <div className="mx-auto max-w-7xl">
        <BrandBanner subtitle="Admin Attendance Page" textClassName="text-orange-900" subtextClassName="text-orange-700" className="mb-4" />
        <button
          type="button"
          onClick={onBack}
          className="mb-4 rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-50"
        >
          Back To Class Page
        </button>

        {loading ? <p className="rounded-lg border border-orange-200 bg-orange-50 px-4 py-3 text-sm text-orange-700">Loading attendance page...</p> : null}
        {error ? <p className="mb-4 rounded-lg border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700">{error}</p> : null}
        {feedback ? <p className="mb-4 rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-700">{feedback}</p> : null}
        ...
        {classroom ? (
          <>
            <section className="rounded-xl bg-slate-900 p-5 text-white">
              ...
              <h1 className="mt-2 text-4xl font-bold">{classroom.className}</h1>
              <p className="mt-2 text-slate-200">{classroom.subject}</p>
              <p className="mt-3 text-sm text-slate-300">{classroom.students.length} students available for today's attendance</p>
            </section>
                    ...
                    <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className="rounded-xl bg-emerald-600 px-3 py-2 text-sm font-semibold text-white transition hover:bg-emerald-700 disabled:bg-emerald-300"
                        >
                        ...
                        <button
                          ...
                          disabled={busyStudent === student.email}
                          className="rounded-xl bg-rose-600 px-3 py-2 text-sm font-semibold text-white transition hover:bg-rose-700 disabled:bg-rose-300"
                        >
