...
function AdminAssignmentPage({ classId, adminUser, assignment, onBack }) {
  const [assignmentData, setAssignmentData] = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');

  async function loadAssignmentData() {
    if (!classId || !adminUser?.email || !assignment) {
      setLoading(false);
      return;
    }
  ...
  useEffect(() => {
    loadAssignmentData();
  }, [classId, adminUser, assignment]);

  return (
    <div className="min-h-screen bg-orange-50 p-5">
      <div className="mx-auto max-w-7xl">
        <BrandBanner subtitle="Admin Assignment Page" textClassName="text-orange-900" subtextClassName="text-orange-700" className="mb-4" />
        <button
          type="button"
          onClick={onBack}
        ...
        {loading ? <p className="rounded-lg border border-orange-200 bg-orange-50 px-3 py-2 text-sm text-orange-700">Loading assignment page...</p> : null}
        ...
        {assignmentData ? (
          <>
            <section className="rounded-xl bg-slate-900 p-5 text-white">
              <p className="text-sm font-semibold uppercase tracking-[0.2em] text-orange-300">Assignment Page</p>
              <h1 className="mt-2 text-4xl font-bold">{assignmentData.assignment.title}</h1>
              <p className="mt-2 text-slate-200">
                {assignmentData.class.className} | {assignmentData.assignment.subject}
              </p>
              <p className="mt-2 text-slate-200">Deadline: {formatDisplayDateTime(assignmentData.assignment.dueDate)}</p>
              <p className="mt-2 text-slate-200">
              ...
              <div className="mt-4 flex flex-wrap gap-3">
                {assignmentData.assignment.attachments?.length ? assignmentData.assignment.attachments.map((attachment) => (
                  <a
                    ...
                    className="rounded-xl border border-orange-200 bg-orange-50 px-3 py-2 text-sm font-medium text-orange-800 transition hover:bg-orange-100"
                  >
